...
BTSiteCamoe.prototype.IsLogin = function (callback) {
  ...
  if (!this.m_cookie) {
    ...
    //check login
    request.setCookie(this.m_cookie, CAMOE_BASE_URL);
    request.get(CAMOE_BASE_URL + '/post.php', {rejectUnauthorized: false}, function (err, response, body) {
      if (err) {
        callback(err);
        return;
      }
...
/*BTSiteCamoe.prototype.GetVcode = function (mode, callback) {
  request.get(CAMOE_BASE_URL + this.m_vcode_url, { buffer: true }, function (err, response, body) {
    if (err) {
      callback('验证码获取失败');
      return;
    }
...
BTSiteCamoe.prototype.LoginEx = function (callback) {
  ...
  request.post(CAMOE_BASE_URL + '/deal.php', formdata,
...
BTSiteCamoe.prototype.LoginSucceed = function (callback) {
  var str_cookie = request.getCookie(CAMOE_BASE_URL);
...
BTSiteCamoe.prototype.UploadEx = function (formdata, callback) {
  var that = this;
  request.post(CAMOE_BASE_URL + '/upload.php',
...
BTSiteCamoe.prototype.getlastpublish = function (callback) {
  request.get(CAMOE_BASE_URL + '/my.php', {rejectUnauthorized: false}, function (err, response, body) {
    if (err) {
      callback(err);
      return;
    }
